...
class ReportsController extends Controller
{
    public function agentKpi()
    {
        ...
        $batchDetails = DB::table('batch_details')
            ...
            ->whereBetween('batch_details.created_at', [$startOfWeek, $endOfWeek])
            ->select(
    ...
    public function getAgentKpiReports(Request $request)
    {
        $page = 'agentkpi'; 
        $companyId = Session::get('company_id');  
        $drivers = Driver::where('company_id', $companyId)->get();
        ...
        // Query the BatchDetails based on driver and date filter with status counts using joins and grouping by driver
        $batchDetails = DB::table('batch_details')
            ->join('drivers', 'batch_details.assignedto', '=', 'drivers.id')
            ->join('batches', 'batch_details.batch_id', '=', 'batches.id')
            ->where('batch_details.assignedto', $driverId)
            ->whereBetween('batch_details.created_at', [$startDate, $endDate])
            ->select(
                'drivers.id as driver_id',
                'drivers.name as driver_name',
                'batches.id as batch_id',
